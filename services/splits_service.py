...
# Module-level split indexes
_season_index: dict[str, dict] = {}    # player_id -> full season averages
_home_index: dict[str, dict] = {}      # player_id -> home averages
_away_index: dict[str, dict] = {}      # player_id -> away averages
_last5_index: dict[str, dict] = {}     # player_id -> last 5 averages
_last10_index: dict[str, dict] = {}    # player_id -> last 10 averages
...
def refresh(season: str = _CURRENT_SEASON, force: bool = False) -> None:
    ...
    """
    global _season_index, _home_index, _away_index, _last5_index, _last10_index, _loaded_date
...
def get_split_context(
    ...
    Computes home/away factors and recent-trend factor.
    """
    ...
    def _get_stat(idx: dict, pid: str) -> float:
        rec = idx.get(pid, {})
        if prop_type == "pra":
            return (
    ...
    season_avg = _get_stat(_season_index, player_id)
    home_avg = _get_stat(_home_index, player_id)
    away_avg = _get_stat(_away_index, player_id)
    last5_avg = _get_stat(_last5_index, player_id)
    last10_avg = _get_stat(_last10_index, player_id)

    home_games = int(_home_index.get(player_id, {}).get("gp", 0) or 0)
    away_games = int(_away_index.get(player_id, {}).get("gp", 0) or 0)
    season_games = int(_season_index.get(player_id, {}).get("gp", 0) or 0)
    ...
    return SplitContext(
        player_id=player_id,
        player_name=_season_index.get(player_id, {}).get("player_name", ""),
        prop_type=prop_type,
...
def enrich_split_context_with_logs(
    ...
    """
    Populate last_5_games, last_10_games, and last_10_std_dev from game logs.

    Call this after get_split_context() to add raw per-game values for
    variance modelling.
    """
    try:
    ...
    for l in logs:
        if prop_type == "pra":
            ...
            vals.append(float(l.get(stat or "pts", 0) or 0))
