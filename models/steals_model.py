"""
Steals projection model  –  Poisson distribution.

Steals are rare discrete events; Poisson is appropriate.
Key factors: opponent turnover rate, player activity level, pace.
"""
...
class StealsModel(BaseStatModel):
    ...
    def _stat_std(self, player: Player, projected_mean: float) -> float:
        return self._variance.std(player, PropType.STEALS, projected_mean)
    ...
    def project(
        ...
        rate, _d = blended_stat_rate(player, PropType.STEALS, exp_minutes)
        ...
        if defense:
            tov_factor = clamp(defense.turnovers_forced_per_game / 14.0, 0.80, 1.20)
        else:
            tov_factor = 1.0

        fpa_factor = self._fpa.factor(defense, player, weight=0.08) if defense else 1.0
        combined_matchup = self._cap_matchup_combined(tov_factor, fpa_factor)
        env = self._environment_multiplier(pace_factor, combined_matchup)

        injury_factor = self._injury_factor(player)

        base = player.steals_per_game or 0.8
        projected = raw_mean * env * injury_factor
        projected = apply_projection_guards(projected, player, PropType.STEALS, exp_minutes)
        projected = clamp(projected, 0.0, min(5.0, self._max_projection(base, player.is_starter)))

        consistency = self._variance.consistency_score(player, PropType.STEALS, projected)
        confidence, _ = self._confidence.score(
            player, PropType.STEALS, projected, consistency,
